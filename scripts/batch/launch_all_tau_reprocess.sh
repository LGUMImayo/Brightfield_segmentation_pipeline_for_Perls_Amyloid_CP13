#!/bin/bash
# ==============================================================================
# Launch All Tau Slides for Reprocessing
# Submits individual SLURM jobs for each slide
# ==============================================================================

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
SUCCESS_FILE="/fslustre/qhs/ext_chen_yuheng_mayo_edu/RO1_GCP/success_tau_stage3.txt"
TAU_BASE_DIR="/fslustre/qhs/ext_chen_yuheng_mayo_edu/RO1_GCP/Pipeline_merged/RO1_Tau/Cases"
...
echo "============================================================"
echo "Tau Reprocessing Launcher"
...
while IFS= read -r SLIDE_NAME; do
    ...
    # Extract case ID (e.g., 2353_22 from 2353_22_#123_MFG_CP13)
    ...
    SLIDE_DIR="$TAU_BASE_DIR/$CASE_ID/$SLIDE_NAME"
    
    if [ ! -d "$SLIDE_DIR" ]; then
        echo "ERROR: Directory not found: $SLIDE_DIR"
        echo "$SLIDE_NAME|DIR_NOT_FOUND" >> "$LOG_DIR/failed_tau_reprocess.txt"
        continue
    fi
    
    echo "Submitting: $SLIDE_NAME"
    sbatch --export=SLIDE_NAME="$SLIDE_NAME",SLIDE_DIR="$SLIDE_DIR" \
        "$SCRIPT_DIR/single_slide_reprocess_tau.sh"
...
echo "Logs: /fslustre/qhs/ext_chen_yuheng_mayo_edu/script/out/reprocess_tau_*.log"
echo "============================================================"

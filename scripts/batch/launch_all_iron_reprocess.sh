#!/bin/bash
# ==============================================================================
# Launch All Iron Slides for Reprocessing
# Submits individual SLURM jobs for each slide
# ==============================================================================

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
SUCCESS_FILE="/fslustre/qhs/ext_chen_yuheng_mayo_edu/RO1_GCP/success_iron_stage3.txt"
IRON_BASE_DIR="/fslustre/qhs/ext_chen_yuheng_mayo_edu/RO1_GCP/Pipeline_merged/RO1_Iron/Cases"
...
echo "============================================================"
echo "Iron Reprocessing Launcher"
...
while IFS= read -r SLIDE_NAME; do
    ...
    # Extract case ID (e.g., 3063_22 from 3063_22_#42_BS_Iron)
    ...
    SLIDE_DIR="$IRON_BASE_DIR/$CASE_ID/$SLIDE_NAME"
    
    if [ ! -d "$SLIDE_DIR" ]; then
        echo "ERROR: Directory not found: $SLIDE_DIR"
        echo "$SLIDE_NAME|DIR_NOT_FOUND" >> "$LOG_DIR/failed_iron_reprocess.txt"
        continue
    fi
    
    echo "Submitting: $SLIDE_NAME"
    sbatch --export=SLIDE_NAME="$SLIDE_NAME",SLIDE_DIR="$SLIDE_DIR" \
        "$SCRIPT_DIR/single_slide_reprocess_iron.sh"
...
echo "Logs: /fslustre/qhs/ext_chen_yuheng_mayo_edu/script/out/reprocess_iron_*.log"
echo "============================================================"

#!/bin/bash
#SBATCH --job-name=reproc_amyloid_%j
#SBATCH --output=/fslustre/qhs/ext_chen_yuheng_mayo_edu/script/out/reprocess_amyloid_%j.log
#SBATCH --error=/fslustre/qhs/ext_chen_yuheng_mayo_edu/script/out/reprocess_amyloid_%j.err
...
# ==============================================================================
# Single Slide Full Reprocessing - Amyloid
# Runs Stage 1 + Stage 2 + Stage 3 from scratch
# ==============================================================================
# Usage: sbatch --export=SLIDE_NAME=<name>,SLIDE_DIR=<path> single_slide_reprocess_amyloid.sh
# ==============================================================================

echo "============================================================"
echo "FULL REPROCESSING: Amyloid"
...
# Paths
PIPELINE_CONFIG="/fslustre/qhs/ext_chen_yuheng_mayo_edu/high-res-3D-tau/pipeline_config_amyloid.txt"
...
# Log files
SUCCESS_LOG="/fslustre/qhs/ext_chen_yuheng_mayo_edu/RO1_GCP/reprocess_logs/success_amyloid_reprocess.txt"
FAILED_LOG="/fslustre/qhs/ext_chen_yuheng_mayo_edu/RO1_GCP/reprocess_logs/failed_amyloid_reprocess.txt"
...
# Clean up mask tiling data to force re-tiling with erosion
echo "Cleaning up mask tiling data (for erosion reprocessing)..."
rm -f "$FILES_DIR/mask/final_mask/mask_tiling_info.xml" 2>/dev/null
rm -rf "$FILES_DIR/mask/final_mask/tiles" 2>/dev/null
rm -f "$FILES_DIR/mask/final_mask/*_full_res.tif" 2>/dev/null
...
# Use predict_iron_effnet.py for Amyloid too (same architecture, different config)
...
if [ ! -d "$FILES_DIR/heatmap/Amyloid_seg_tiles" ]; then
    echo "STAGE 2 FAILED - No Amyloid_seg_tiles created"
    echo "$SLIDE_NAME|STAGE2_FAILED" >> "$FAILED_LOG"
    exit 1
fi

PRED_TILE_COUNT=$(find "$FILES_DIR/heatmap/Amyloid_seg_tiles" -name "*_mask.tif" 2>/dev/null | wc -l)
if [ "$PRED_TILE_COUNT" -eq 0 ]; then
    echo "STAGE 2 FAILED - Amyloid_seg_tiles empty"
...
python /fslustre/qhs/ext_chen_yuheng_mayo_edu/rhizonet/rhizonet/mask_predictions_to_tissue.py \
    --pred_dir "$FILES_DIR/heatmap/Amyloid_seg_tiles" \
...
python /fslustre/qhs/ext_chen_yuheng_mayo_edu/high-res-3D-tau/pipeline/run_pipeline_part2.py \
    "$FILES_DIR" "$PIPELINE_CONFIG" "Amyloid" 2>&1
...
# ==============================================================================
# CLEANUP: Remove intermediate files (DISABLED FOR TESTING)
# ==============================================================================
echo ""
echo "============================================================"

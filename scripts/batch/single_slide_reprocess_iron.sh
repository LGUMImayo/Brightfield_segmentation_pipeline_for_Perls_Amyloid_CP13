#!/bin/bash
#SBATCH --job-name=reproc_iron_%j
#SBATCH --output=/fslustre/qhs/ext_chen_yuheng_mayo_edu/script/out/reprocess_iron_%j.log
#SBATCH --error=/fslustre/qhs/ext_chen_yuheng_mayo_edu/script/out/reprocess_iron_%j.err
...
# ==============================================================================
# Single Slide Full Reprocessing - Iron
# Runs Stage 1 + Stage 2 + Stage 3 from scratch
# ==============================================================================
# Usage: sbatch --export=SLIDE_NAME=<name>,SLIDE_DIR=<path> single_slide_reprocess_iron.sh
# ==============================================================================

echo "============================================================"
echo "FULL REPROCESSING: Iron"
...
PREDICTION_CONFIG="/fslustre/qhs/ext_chen_yuheng_mayo_edu/rhizonet/data/setup_files/setup-predict_Iron_efficientnet_v2.json"
FILES_DIR="${SLIDE_DIR}/${SLIDE_NAME}_files"

# Log files
SUCCESS_LOG="/fslustre/qhs/ext_chen_yuheng_mayo_edu/RO1_GCP/reprocess_logs/success_iron_reprocess.txt"
FAILED_LOG="/fslustre/qhs/ext_chen_yuheng_mayo_edu/RO1_GCP/reprocess_logs/failed_iron_reprocess.txt"
...
rm -rf "$FILES_DIR/heatmap/Iron_seg_tiles" 2>/dev/null
...
python /fslustre/qhs/ext_chen_yuheng_mayo_edu/rhizonet/rhizonet/predict_iron_effnet.py \
    ...
    --output_folder Iron_seg_tiles 2>&1

END_STAGE2=$(date +%s)
ELAPSED2=$((END_STAGE2 - START_STAGE2))

# Verify Stage 2 output
if [ ! -d "$FILES_DIR/heatmap/Iron_seg_tiles" ]; then
    echo "STAGE 2 FAILED - No Iron_seg_tiles created"
    echo "$SLIDE_NAME|STAGE2_FAILED" >> "$FAILED_LOG"
    exit 1
fi

PRED_TILE_COUNT=$(find "$FILES_DIR/heatmap/Iron_seg_tiles" -name "*_mask.tif" 2>/dev/null | wc -l)
if [ "$PRED_TILE_COUNT" -eq 0 ]; then
    echo "STAGE 2 FAILED - Iron_seg_tiles empty"
...
python /fslustre/qhs/ext_chen_yuheng_mayo_edu/rhizonet/rhizonet/mask_predictions_to_tissue.py \
    --pred_dir "$FILES_DIR/heatmap/Iron_seg_tiles" \
...
python /fslustre/qhs/ext_chen_yuheng_mayo_edu/high-res-3D-tau/pipeline/run_pipeline_part2.py \
    "$FILES_DIR" "$PIPELINE_CONFIG" "Iron" 2>&1
...
if [ $VERIFY_CODE -eq 1 ]; then
    ...
    # Don't exit - still proceed with cleanup
fi

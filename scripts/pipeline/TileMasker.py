...
class TileMasker(object):
    ...
    def apply_masks(self,root_dir):
        ...
        for ldir in list_dirs:
            ...
            if os.path.isdir(ldir):
                ...
                histo_dir = ''
                seg_dir = os.path.join(ldir,'heatmap/seg_tiles')
                ...
                if nT_mask != nT_histo:
                    ...
                    print('ERROR: the number of mask and histology tiles is different. Skipping this images.')
                    continue
                ...
                for mTile in tiles_mask:
                    ...
                    if not (mask.shape[0] == img.shape[0] and mask.shape[1] == img.shape[1]):
                        # Use cv2.resize for better performance and reliability
                        try:
                            import cv2
                            ...
                            mask = skimage.transform.resize(mask, (img.shape[0], img.shape[1]), order=0, preserve_range=True).astype(mask.dtype)
    ...
    def create_dir_struct(self,root_dir):
        # --- Check for and remove old 'heat_map' directory with underscore ---
        old_hmap_dir = os.path.join(root_dir, 'heat_map')
        if os.path.exists(old_hmap_dir) and os.path.isdir(old_hmap_dir):
            self.logger.info(f"Removing old directory structure: {old_hmap_dir}")
            print(f"Removing old directory: {old_hmap_dir}")
            shutil.rmtree(old_hmap_dir)

        hmap_dir = os.path.join(root_dir,'heatmap')
